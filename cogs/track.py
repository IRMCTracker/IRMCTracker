from modules.classes.server import Server
...
class Track(Cog):
    ...
    async def servers(self, ctx):
        ...
        servers = [Server(server_data) for server_data in servers]
        banner = File('storage/static/banner.png', filename='banner.png')
        embed = Embed(title="📡 Servers List | لیست سرور ها",
                      description='', color=0x673AB7)
        for server in servers:
            embed.add_field(
                name=server.name, value=f"👥 {server.current_players}", inline=True)
    ...
    async def track(self, ctx, server=None):
        ...
        if not type(servers_alike) == type([]) or len(servers_alike) == 0:
        ...
        servers_alike = [Server(server_data) for server_data in servers_alike]

        if len(servers_alike) > 1:
            alike_names = []
            ...
            return await ctx.send(mention_msg, embed=Embed(title='Server morede nazar peida nashod!',
                                                           description='Shayad manzooretoon yeki az in hast: **' +
                                                           ' | '.join(
                                                               alike_names) + '**',
                                                           color=0xcddc39))
            ...
            if server['latest_latency'] == 0:
                embed = Embed(
                    title=f"🔴 {server.name}", description='Server morede nazar shoma dar hale hazer offline hast : (', color=0xc62828)
